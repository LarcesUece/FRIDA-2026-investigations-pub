...
def mapear_colunas(colunas):
    ...
    return {
        'drop': [c for c in colunas if c.split('_')[0] in ['nome', 'estado', 'email', 'sexo']],
...
def testar_pandas(arquivo, res):
    ...
    with Monitor(res, 'Pandas', '7_Write'):
        ...
        caminho_saida = arquivo.replace('DATA/', 'DATA/trash/').replace('.csv', '_pd.csv')
...
def testar_polars(arquivo, res):
    ...
    with Monitor(res, 'Polars', '3_Mask (Pseudonimização)'):
        expr_cpf = [(pl.lit("***.***.***-") + pl.col(c).cast(pl.Utf8).str.slice(-2)).alias(c) for c in cols['cpf']]
    ...
    with Monitor(res, 'Polars', '7_Write'):
        caminho_saida = arquivo.replace('DATA/', 'DATA/trash/').replace('.csv', '_pl.csv')
...
def testar_duckdb(arquivo, res):
    ...
    with Monitor(res, 'DuckDB', '3_Mask (Pseudonimização)'):
        for c in cols['cpf']:
            conn.sql(f"UPDATE t SET {c} = '***.***.***-' || substr(CAST({c} AS VARCHAR), -2)")
    ...
    with Monitor(res, 'DuckDB', '6_Stats (Descritivas)'):
        for c in cols['diff_priv']:
            conn.sql(f"SELECT avg({c}), stddev({c}), quantile_cont({c}, 0.5) FROM t").fetchall()
            
    with Monitor(res, 'DuckDB', '7_Write'):
        caminho_saida = arquivo.replace('DATA/', 'DATA/trash/').replace('.csv', '_duck.csv')
        conn.sql(f"COPY t TO '{caminho_saida}' (HEADER, DELIMITER ',')")
    conn.close(); del conn; gc.collect()
...
def testar_pyarrow(arquivo, res):
    ...
    with Monitor(res, 'PyArrow', '3_Mask (Pseudonimização)'):
        for c in cols['cpf']:
            ...
            cpf_masc = pc.replace_substring_regex(str_col, pattern=r'^.{12}', replacement='***.***.***-')
            tabela = tabela.set_column(tabela.schema.get_field_index(c), pa.field(c, pa.string()), cpf_masc)
            
    with Monitor(res, 'PyArrow', '4_K-Anonymity (Peso)'):
        for c in cols['k_anon']:
            k_anon = pc.multiply(pc.floor(pc.divide(tabela[c], 10)), 10)
            tabela = tabela.set_column(tabela.schema.get_field_index(c), pa.field(c, pa.float64()), pc.cast(k_anon, pa.float64()))
    ...
    with Monitor(res, 'PyArrow', '5_Diff Privacy (Valor Precisão)'):
        ...
        for c in cols['diff_priv']:
            ...
            tabela = tabela.set_column(tabela.schema.get_field_index(c), pa.field(c, pa.float64()), val)
            
    with Monitor(res, 'PyArrow', '6_Stats (Descritivas)'):
        for c in cols['diff_priv']:
            pc.mean(tabela[c]); pc.stddev(tabela[c]); pc.quantile(tabela[c], q=[0.25, 0.5, 0.75])
            
    with Monitor(res, 'PyArrow', '7_Write'):
        caminho_saida = arquivo.replace('DATA/', 'DATA/trash/').replace('.csv', '_arrow.csv')
        pv.write_csv(tabela, caminho_saida)
    del tabela; gc.collect()
...
def testar_dask_free(arquivo, res):
    ...
    with Monitor(res, 'Dask_Free', '5_Diff Privacy (Valor Precisão)'):
        def aplicar_ruido(s): return s + np.random.laplace(0.0, 50.0, len(s))
        for c in cols['diff_priv']:
    ...
    with Monitor(res, 'Dask_Free', '7_Write (Execução Total)'):
        caminho_saida = arquivo.replace('DATA/', 'DATA/trash/').replace('.csv', '_dask_free_out_*.csv')
...
def rodar_pipeline_completo(arquivo_teste):
    ...
    nome_csv = arquivo_teste.replace('.csv', '_RELATORIO.csv')
